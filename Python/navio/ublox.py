...
class UBloxDescriptor:
    ...
    def __init__(
        self, name, msg_format, fields=[], count_field=None, format2=None, fields2=None
    ):
        self.name = name
...
msg_types = {
    (CLASS_ACK, MSG_ACK_ACK): UBloxDescriptor("ACK_ACK", "<BB", ["clsID", "msgID"]),
    (CLASS_ACK, MSG_ACK_NACK): UBloxDescriptor("ACK_NACK", "<BB", ["clsID", "msgID"]),
    ...
    (CLASS_CFG, MSG_CFG_CFG): UBloxDescriptor(
        "CFG_CFG", "<III,B", ["clearMask", "saveMask", "loadMask", "deviceMask"]
    ),
    (CLASS_CFG, MSG_CFG_RST): UBloxDescriptor(
        "CFG_RST", "<HBB", ["navBbrMask ", "resetMode", "reserved1"]
    ),
    (CLASS_CFG, MSG_CFG_SBAS): UBloxDescriptor(
        "CFG_SBAS", "<BBBBI", ["mode", "usage", "maxSBAS", "scanmode2", "scanmode1"]
    ),
    ...
    (CLASS_NAV, MSG_NAV_PVT): UBloxDescriptor(
        "NAV_PVT",
    ...
    (CLASS_NAV, MSG_NAV_POSUTM): UBloxDescriptor(
        "NAV_POSUTM", "<Iiiibb", ["iTOW", "East", "North", "Alt", "Zone", "Hem"]
    ),
    ...
    (CLASS_NAV, MSG_NAV_POSECEF): UBloxDescriptor(
        "NAV_POSECEF", "<IiiiI", ["iTOW", "ecefX", "ecefY", "ecefZ", "pAcc"]
    ),
    (CLASS_NAV, MSG_NAV_VELECEF): UBloxDescriptor(
        "NAV_VELECEF", "<IiiiI", ["iTOW", "ecefVX", "ecefVY", "ecefVZ", "sAcc"]
    ),
    (CLASS_NAV, MSG_NAV_TIMEGPS): UBloxDescriptor(
        "NAV_TIMEGPS", "<IihbBI", ["iTOW", "fTOW", "week", "leapS", "valid", "tAcc"]
    ),
    ...
    (CLASS_NAV, MSG_NAV_CLOCK): UBloxDescriptor(
        "NAV_CLOCK", "<IiiII", ["iTOW", "clkB", "clkD", "tAcc", "fAcc"]
    ),
    ...
    (CLASS_RXM, MSG_RXM_EPH): UBloxDescriptor(
        "RXM_EPH", "<II , 8I 8I 8I", ["svid", "how", "sf1d[8]", "sf2d[8]", "sf3d[8]"]
    ),
    (CLASS_AID, MSG_AID_EPH): UBloxDescriptor(
        "AID_EPH", "<II , 8I 8I 8I", ["svid", "how", "sf1d[8]", "sf2d[8]", "sf3d[8]"]
    ),
    ...
    (CLASS_TIM, MSG_TIM_TP): UBloxDescriptor(
        "TIM_TP", "<IIiHBB", ["towMS", "towSubMS", "qErr", "week", "flags", "reserved1"]
    ),
...
class UBloxMessage:
    ...
    def unpack(self):
        ...
        if not type in msg_types:
            raise UBloxError(
                "Unknown message %s length=%u" % (str(type), len(self._buf))
            )
    ...
    def valid_so_far(self):
        ...
        if self.needed_bytes() == 0 and not self.valid():
            if len(self._buf) > 8:
                ...
                self.debug(
                    1,
                    "bad len len=%u needed=%u" % (len(self._buf), self.needed_bytes()),
                )
            return False
        return True
    ...
    def valid(self):
        ...
        return (
            len(self._buf) >= 8 and self.needed_bytes() == 0 and self.valid_checksum()
        )
...
class UBlox:
    ...
    def special_handling(self, msg):
        ...
        if msg.name() == "CFG_NAV5":
            ...
            pollit = False
            if (
                self.preferred_dynamic_model is not None
                and msg.dynModel != self.preferred_dynamic_model
            ):
                msg.dynModel = self.preferred_dynamic_model
                sendit = True
                pollit = True
            if (
                self.preferred_dgps_timeout is not None
                and msg.dgpsTimeOut != self.preferred_dgps_timeout
            ):
    ...
    def configure_port(self, port=1, inMask=3, outMask=3, mode=2240, baudrate=None):
        ...
        if baudrate is None:
            baudrate = self.baudrate
        payload = struct.pack(
            "<BBHIIHHHH", port, 0xFF, 0, mode, baudrate, inMask, outMask, 0xFFFF, 0xFFFF
        )
